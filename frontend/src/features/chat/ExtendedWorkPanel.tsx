...
export function ExtendedWorkPanel({
  ...
  const complete = tasks.filter((task) => ['completed', 'blocked'].includes(task.status)).length;
  const remaining = tasks.length - complete;
  const percent = Math.round(100 * complete / tasks.length);

  return (
    <details className={`extended-work${sidebar ? ' sidebar' : ''}`} open={sidebar || complete < tasks.length}>
      <summary>
        ...
        {tasks.map((task) => {
          ...
          return (
            <div className={`extended-work-item ${task.status}`} key={task.id}>
              <span className="extended-work-status" aria-hidden="true">
