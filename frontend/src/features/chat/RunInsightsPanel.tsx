...
export function RunInsightsPanel({
  ...
  const epochCount = events.filter((event) => event.event_type === 'run.epoch.started').length;
  const retryCount = events.filter(
    (event) => event.event_type === 'tool.attempt.started'
      && typeof event.payload.attempt === 'number'
      && event.payload.attempt > 1,
  ).length;
  const compactionCount = events.filter((event) => event.event_type === 'context.compacted').length;
      ...
      <div className="run-insights-content">
        {activeTab === 'overview' ? (
          <section className="context-usage-card" aria-label="Token usage">
            ...
            <div className="context-usage-breakdown">
              <span>Epochs<strong>{epochCount || 1}</strong></span>
              <span>Retries<strong>{retryCount}</strong></span>
              <span>Compactions<strong>{compactionCount}</strong></span>
            </div>
          </section>
        ) : null}
...
function latestTaskCount(events: readonly RunStreamEvent[]): number {
  for (let index = events.length - 1; index >= 0; index -= 1) {
    ...
    if (event.event_type === 'goal.plan.updated' && Array.isArray(event.payload.steps)) {
      return event.payload.steps.length;
    }
  }
  return 0;
}
