"""Runs user-supplied Python for a FunctionTool in a constrained subprocess.
...
def _no_network(*args, **kwargs):
    raise PermissionError("Network access is disabled in the Python tool sandbox")
...
def _guarded_import(name, globals=None, locals=None, fromlist=(), level=0):
    ...
    if importer == "__sandbox__" and not _permitted(name):
        raise ImportError(
            "Importing '%s' is not allowed in the Python tool sandbox. "
...
def _guarded_open(file, mode="r", *args, **kwargs):
    try:
        ...
        raise PermissionError("The Python tool sandbox only opens paths inside its scratch directory")
    if not (target == _scratch_real or target.startswith(_scratch_real + os.sep)):
        raise PermissionError(
            "The Python tool sandbox may only read and write inside its scratch directory (%s)" % scratch
        )
...
kind = "error"
try:
    exec(compile(payload["code"], "<python tool>", "exec"), namespace)
...
try:
    # default=str keeps a stray datetime or custom object from failing the whole tool;
...
async def run_python(
    ...
    *,
    ...
    scratch = Path(tempfile.mkdtemp(prefix="python-tool-"))
    payload = json.dumps(
        {
            "code": code,
            "inputs": inputs,
