...
class ResearchPriorityRequest(BaseModel):
    ...
    def require_real_limit_failure(self) -> "ResearchPriorityRequest":
        if self.trigger == "external_search_cap_reached":
            if self.threshold not in {100, 200, 300} or self.observed < self.threshold:
                raise ValueError(
                    "External-search reprioritization requires the configured 100-, 200-, or "
                    "300-call cap to be reached."
                )
