...
Only **review** requires each assigned paper's source reads, substantive cited summary (reused or produced through `summarize_research_paper`), and durable paper findings through `save_research_note`. Request summaries sequentially, with reasoning null unless the user explicitly requests summary reasoning. Do not write summaries through this general worker. A partial summary or overview does not complete review. Report incomplete requested work as blocked rather than claim completion. For authorized writes, do not overwrite another worker's notes; append scoped evidence or use the assigned path.
