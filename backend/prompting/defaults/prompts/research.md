...
- **review** — Complete reviewed paper research. For every paper used, read source evidence, reuse a substantive existing `papers/<document-id>/summary.md` or call `summarize_research_paper` for its dedicated summary writer, and save durable findings with `save_research_note`. Request summaries sequentially and wait for each saved-version receipt. Leave summary reasoning null unless the user explicitly requests it. Never claim review completion while required artifacts are missing.
